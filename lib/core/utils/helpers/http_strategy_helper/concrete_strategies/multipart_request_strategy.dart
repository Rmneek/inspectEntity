...
class MultipartPostRequestStrategy implements HttpRequestStrategy {
  ...
  Future<ApiResultModel<http.Response>> executeRequest({
    ...
    try {
      AppLogger.debug('📤 MultipartPostRequestStrategy.executeRequest called');
      AppLogger.debug('➡️ URI: $uri');
      AppLogger.debug('➡️ Headers: $headers');
      AppLogger.debug('➡️ RequestData: $requestData');
      ...
      AppLogger.debug('📎 Detected MIME type: $mimeType');
      ...
      AppLogger.debug('Uploading file: ${file.path}');
      final streamedResponse = await request.send().timeout(timeOutDuration);

      final response = await http.Response.fromStream(streamedResponse);
      AppLogger.debug('Upload Response: ${response.body}');

      return response.performHttpRequest();
    } catch (e) {
      AppLogger.debug(' Multipart upload error: $e');
